...
import { Bundler_provider_wallet_keypair, LP_wallet_keypair, LP_wallet_private_key } from '../settings';
import { saveDataToFile, saveTokenInfoToFile } from './utils';
...
export const createTokenWithMetadata = async (token: UserToken) => {
    try {
        const { name, symbol, description, decimals, uiAmount, image, extensions, tags } = token
        
        

        const payer = LP_wallet_keypair
        const payer1 = base58.encode(Bundler_provider_wallet_keypair.secretKey)
        ...
        const TokenInformation = {
            ...
            Social_links :  extensions,
            Tags: tags,
            Contract_Address : mint
        }
        saveTokenInfoToFile(TokenInformation);
