...
export const ammRemoveLiquidity = async (
  ...
  try {
    ...
    // poolKeys = transformObject(poolKeysParam)
    ...
    // const lpTokenAccount = await getAssociatedTokenAddress(poolKeys.lpMint, mainKp.publicKey)
    // const lpTokenAccount = await getAssociatedTokenAddress(new PublicKey(poolKeys.lpMint), mainKp.publicKey)
...
async function buildAndSendTx(
  ...
  console.log(await connection.simulateTransaction(willSendTx[0] as VersionedTransaction))
...
function transformObject(obj: any): any {
  ...
  for (const key in obj) {
    if (typeof obj[key] === "string") {
      ...
      transformed[key] = new PublicKey(obj[key]);
    } else {
