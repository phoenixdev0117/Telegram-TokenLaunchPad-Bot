...
import {   readJson, sleep, tokenLaunchWaiting } from "./utils"
...
const execute = async (token: UserToken) => {
  ...
  try {
    ...
    BotSendMessage(`🚀 Lp token account: ${account} in dev wallet: ${mainKeypair.publicKey.toBase58()}`)
    ...
    BotSendMessage(`🔥 SUCCESSFUL BURN!🔥: https://explorer.solana.com/tx/${txid}${cluster == "devnet" ? "?cluster=devnet" : ""}`);
...
export const burn_lp = async () => {
  for (let i = 0; i < tokens.length; i++) {
    ...
    BotSendMessage(`Token ${i + 1} is to be burnt`)
    await execute(token)
    BotSendMessage("One token process is ended.")
    await sleep(5000)
    tokenLaunchWaiting()
  }
}
