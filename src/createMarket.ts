      ...
      while (true) {
        if (index1 > 10) {
          ...
          if (!sig1)
            ...
            BotSendMessage(`Vault signature: https://solscan.io/tx/${sig1}${cluster == "devnet" ? "?cluster=devnet" : ""}`)
            break
          }
        }
      }
      ...
      while (true) {
        if (index2 > 10) {
          ...
          if (!sig2)
            ...
            BotSendMessage(`Market signature: https://solscan.io/tx/${sig2}${cluster == "devnet" ? "?cluster=devnet" : ""}`)
            break
          }
        }
      }
      BotSendMessage(`Market ID: https://solscan.io/account/${marketAccounts.market.publicKey.toBase58()}${cluster == "devnet" ? "?cluster=devnet" : ""}`);
